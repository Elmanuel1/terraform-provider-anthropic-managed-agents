...
func NewAgentClient(cfg *Config, workspaceID string) *AgentClient {
	return &AgentClient{cfg: cfg, workspaceID: workspaceID}
}
...
func (c *AgentClient) Create(ctx context.Context, body map[string]any) (*AgentResponse, error) {
	raw, status, err := doWithCreds(ctx, c.cfg, c.creds(), http.MethodPost, "/v1/agents", body)
	if err != nil {
...
func (c *AgentClient) Read(ctx context.Context, id string) (*AgentResponse, error) {
	raw, status, err := doWithCreds(ctx, c.cfg, c.creds(), http.MethodGet, "/v1/agents/"+id, nil)
	if err != nil {
...
func (c *AgentClient) Update(ctx context.Context, id string, body map[string]any) (*AgentResponse, error) {
	raw, status, err := doWithCreds(ctx, c.cfg, c.creds(), http.MethodPost, "/v1/agents/"+id, body)
	if err != nil {
...
func (c *AgentClient) Delete(ctx context.Context, id string) error {
	_, status, err := doWithCreds(ctx, c.cfg, c.creds(), http.MethodPost, "/v1/agents/"+id+"/archive", nil)
	if err != nil {
